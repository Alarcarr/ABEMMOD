...
class StrategicIconNodeScript : StrategicIcon {
	...
	void establish(Node& node, Object@ obj, double Size, SpriteSheet& sht, uint index) {
		@sheet = sht;
		icon = index;
		size = Size;
		establish(node, obj);
	...
	bool preRender(Node& node) {
		...
		if(objDist > 1.0e4 * object.radius)
			return false;
		...
		if(isMemory)
			alpha *= 0.7f;
		if(objDist > 0.8e4 * object.radius)
			alpha *= 1.0 - (objDist - 0.8e4 * object.radius) / (0.2e4 * object.radius);
