...
class ExpendResources : Action {
	...
	bool spendResearch(BasicAI@ ai) {
		...
		for(uint i = 0, cnt = techs.length / (7 - ai.skillEconomy) + 1; i < cnt; ++i) {
			...
			if(!tech.available || tech.bought)
				continue;
			if(tech.type.pointCost > 0 && pts > tech.type.pointCost) {
	...
	bool perform(BasicAI@ ai) {
		switch(nextStep++ % 9) {
			case 0:
				if(ai.empire.Influence > 0)
					if(spendInfluence(ai))
						return false;
			case 1:
			...
			case 3:
			{
				...
				if(ai.enemies.length != 0) {
					...
					if(ai.treatyWaits[other.index] < gameTime) {
						...
						if((ai.empire.points.value + 200) / (other.points.value + 200 + relation) >= 5) {
							demandSurrender(ai.empire, other);
							treated = true;
						}
						else if((other.points.value + 250 + relation) / (ai.empire.points.value + 250) >= 5 && ai.empire.MilitaryStrength < other.MilitaryStrength) {
							...
							if(weakest is other && (totalPoints + 150) / (ai.empire.points.value + 150) >= 2) {
