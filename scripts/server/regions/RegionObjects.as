...
#section all
	...
	void updateContested(Object& region) {
		...
		for(uint i = 0; i < empCnt; ++i) {
			...
			// and an empire hostile to it has planets or military.
			Empire@ other = getEmpire(i);
			int hostile = other.hostileMask;

			bool contested = (planetCounts[i] > 0 || strengths[i] > 0)
				&& (hostile & HasMilitaryMask != 0 || hostile & HasPlanetsMask != 0);
			if(contested && reg.CombatMask & other.mask != 0) {
				CMask |= other.mask;
...
#section server
	...
	void refreshSupportsFor(Object& dest, bool keepGhosts) {
		...
		for(uint i = 0, cnt = planetList.length; i < cnt; ++i) {
			...
			if(pl.owner is owner && pl.SupplyUsed > 0)
...
#section all
		...
		switch(obj.type) {
			case OT_Planet:
				if(prevOwner !is null && prevOwner.valid) {
					...
					strengths[prevOwner.index] -= PLANET_STRENGTH;
				}
				
				if(newOwner !is null && newOwner.valid) {
					planetCounts[newOwner.index] += 1;
					strengths[newOwner.index] += PLANET_STRENGTH;
	...
	void leaveRegion(Object& thisObj, Object& obj) {
		...
		if(ship !is null) {
			...
			if(obj.owner !is null && obj.owner.valid && !dsg.hasTag(ST_Support))
			...
			if(pl !is null) {
				...
				if(obj.owner !is null && obj.owner.valid) {
					...
					strengths[obj.owner.index] -= PLANET_STRENGTH;
				}
				calculatePlanets(region);
			}
			else {
...
#section all
	...
	void calculateVision(Object& obj) {
		...
		for(uint i = 0, cnt = planetList.length; i < cnt; ++i) {
			...
			if(owner is null || !owner.valid)
				continue;
			if(pl.supportCount > 0)
				plSupMask |= owner.mask;
		}
