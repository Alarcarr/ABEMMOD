...
void createColonizer(Object& from, Object& to, double population, double accMod) {
	ObjectDesc colDesc;
	...
	colShip.maxAcceleration = COLONYSHIP_BASE_ACCEL * accMod * from.owner.ModSpeed.value;
	colShip.Health *= from.owner.ModHP.value;
	colShip.finalizeCreation();
}

void createColonizer(Empire@ owner, vec3d from, Object& to, double population, double accMod) {
	ObjectDesc colDesc;
	colDesc.type = OT_ColonyShip;
	colDesc.name = locale::COLONY_SHIP;
	colDesc.radius = 4.0 * pow(population / 0.0625, 1.0 / 3.0);
	...
	colShip.CarriedPopulation = population;
	colShip.maxAcceleration = COLONYSHIP_BASE_ACCEL * accMod * owner.ModSpeed.value;
