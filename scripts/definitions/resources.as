...
import double getPlanetLevelRequiredPop(uint level) from "planet_levels";
...
const double[] RARITY_DISTRIBUTION = {
	...
	0.80, 0.13, 0.05, 0.02, 0.05, 0.00002, //Level 1
	0.75, 0.15, 0.06, 0.04, 0.05, 0.00002, //Level 2
...
void getBuildCost(const Design@ dsg, int&out buildCost, int&out maintainCost, double&out laborCost, int count = 1, Object@ obj = null) {
	...
	buildCost = ceil(build);
	maintainCost = ceil(maintain);
...
class Resources : Serializable, Savable {
	array<uint> types;
	int[] amounts;
...
class Resource : Serializable, Savable {
	int id = -1;
...
class QueuedResource : Serializable, Savable {
	Empire@ forEmpire;
	int id = -1;
	Object@ to;
	bool locked;
...
class AutoImportDesc : Serializable {
	Object@ to;
	const ResourceType@ type;
	const ResourceClass@ cls;
	int level = -1;
