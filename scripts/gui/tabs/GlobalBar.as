...
class ResearchResource : ResourceDisplay {
	...
	void update() {
		...
		if(researching.length != 0) {
			...
			techBar.progress = 1.0 - (activeTech.timer / activeTech.getTimeCost());
		}
		else {
...
class DefenseResource : ResourceDisplay {
	...
	string get_tooltip() {
		return format(locale::GTT_DEFENSE, standardize(playerEmpire.globalDefenseRate * 60.0, true)+locale::PER_MINUTE);
	}

	void update() {
		double income = playerEmpire.globalDefenseRate;
		upperText.text = format(
				"$1[color=#aaa][vspace=6][font=Normal]$2[/font][/vspace][/color]",
				standardize(income * 60.0, true), locale::PER_MINUTE);
		ResourceDisplay::update();
