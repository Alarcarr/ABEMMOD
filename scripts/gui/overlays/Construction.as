					...
					if(ele !is null) {
						if(ele.dsg !is null) {
							if(ele.dsg.hasTag(ST_Support)) {
								...
								list.clearSelection();
								overlay.close();
								targetPoint(OrbitalTarget(obj, null, ele.dsg));
							}
							else {
								if(!obj.canBuildShips)
									return true;
							...
							if(ele.orb.isCore) {
								...
								list.clearSelection();
								overlay.close();
								targetPoint(OrbitalTarget(obj, ele.orb));
							}
							else {
...
class OrbitalTarget : PointTargeting {
	...
	OrbitalTarget(Object@ obj, const OrbitalModule@ mod, const Design@ dsg = null, bool wasShifted = false) {
		@this.obj = obj;
	...
	bool valid(const vec3d& pos) override {
		...
		if(dsg !is null)
			return canBuildOrbital(obj, pos);
		return def.canBuild(obj, pos);
	}

	void call(const vec3d& pos) override {
		if(dsg !is null)
			obj.buildStation(dsg, pos, frame=hoveredObject);
		else
			obj.buildOrbital(def.id, pos, frame=hoveredObject);
		if(shiftKey)
			targetPoint(OrbitalTarget(obj, def, dsg, true));
	}
...
class BuildElement : GuiListElement {
	...
	void update(ConstructionDisplay@ disp) {
		...
		if(dsg !is null) {
			...
			build = building.buildCostEst;
			build *= buildAt.constructionCostMod;
			maintain = building.maintainCostEst;
			if(building.laborCost > 0)
				labor = building.laborCost;
			else
				time = building.buildTime / (buildAt.buildingConstructRate * buildAt.owner.BuildingConstructRate * buildAt.owner.ImperialBldConstructionRate);
	...
	void draw(GuiListbox@ ele, uint flags, const recti& pos) override {
		...
		if(laborText.length != 0) {
			if(costText.length == 0 || extraCosts.length == 0)
				x -= 120;
			else
				x -= 60;
			...
			if(timeText.length != 0 && (costText.length == 0 || extraCosts.length == 0)) {
				font.draw(pos=recti_area(vec2i(x+28, 0)+pos.topLeft, vec2i(92, 30)),
